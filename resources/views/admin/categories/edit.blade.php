...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.categories.update", [$category->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="photos">{{ trans('cruds.category.fields.photos') }}</label>
                <div class="needsclick dropzone {{ $errors->has('photos') ? 'is-invalid' : '' }}" id="photos-dropzone">
                </div>
                @if($errors->has('photos'))
                    <span class="text-danger">{{ $errors->first('photos') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.category.fields.photos_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="ar_title">{{ trans('cruds.category.fields.ar_title') }}</label>
                <input class="form-control {{ $errors->has('ar_title') ? 'is-invalid' : '' }}" type="text" name="ar_title" id="ar_title" value="{{ old('ar_title', $category->ar_title) }}">
                @if($errors->has('ar_title'))
                    <span class="text-danger">{{ $errors->first('ar_title') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.category.fields.ar_title_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="ar_description">{{ trans('cruds.category.fields.ar_description') }}</label>
                <textarea class="form-control {{ $errors->has('ar_description') ? 'is-invalid' : '' }}" name="ar_description" id="ar_description">{{ old('ar_description', $category->ar_description) }}</textarea>
                @if($errors->has('ar_description'))
                    <span class="text-danger">{{ $errors->first('ar_description') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.category.fields.ar_description_helper') }}</span>
            </div>
            <div class="form-group">
...
Dropzone.options.photosDropzone = {
    ...
    init: function () {
@if(isset($category) && $category->photos)
      var files = {!! json_encode($category->photos) !!}
          for (var i in files) {
          var file = files[i]
          this.options.addedfile.call(this, file)
          this.options.thumbnail.call(this, file, file.preview)
